/*
 * Copyright (C) 2020-2022 The DotOS Project
 *
...
import com.dot.systemupdates.ui.ChangelogSheet
import com.google.android.material.chip.Chip
import com.google.android.material.snackbar.Snackbar
import com.dot.systemupdates.changelog.ChangelogAdapter
import com.dot.systemupdates.controller.UpdaterController
import com.dot.systemupdates.controller.UpdaterService
import com.dot.systemupdates.misc.BuildInfoUtils
import com.dot.systemupdates.misc.Constants
import com.dot.systemupdates.misc.StringGenerator
...
import com.dot.systemupdates.model.UpdateStatus
import java.io.File
...
class UpdateView : LinearLayout {
    ...
    fun unleashTheBunny(resID: Int) {
        ...
        Handler(context.mainLooper).postDelayed(
            { findViewById<TextView>(R.id.easterBunny).setText(R.string.maybe_later) },
            4000
        )
    }
    ...
    fun lateInit() {
        ...
        if (activeLayout) handleActiveStatus(update!!) else handleNotActiveStatus(update!!)
        parseChangelog()
    }
    ...
    private fun parseChangelog() {
        ...
        moreChangelog.isEnabled = changelog != null
        if (changelog != null) {
            ...
            for (i in 0 until list.size) {
                with(list[i]) {
                    if (subtitle != "null" && subtitle != "") {
                        summaryChangelogText += "∘ ${subtitle?.replace(".", "")}." +
                                (if (i < list.size - 1) "\n" else "")
                    }
                }
            }
            summaryChangelog.visibility =
                if (summaryChangelogText.isEmpty()) View.GONE else View.VISIBLE
            summaryChangelog.text = summaryChangelogText
            moreChangelog.setOnClickListener {
                ChangelogSheet().setupChangelogSheet(adapter)
                    .show(mActivity!!.supportFragmentManager, "changelog")
            }
        } else {
            summaryChangelog.text = context.getString(R.string.changelog_error)
            Log.w("Changelog", "Couldn't find any changelog")
        }
    }
    ...
    private fun handleActiveStatus(update: UpdateInfo) {
        ...
        when {
            ...
            mUpdaterController!!.isVerifyingUpdate(downloadId) -> {
                ...
                val percentage = NumberFormat.getPercentInstance().format(
                    (update.progress / 100f).toDouble())
                actionProgressStats.text = mActivity!!.getString(
                    R.string.list_download_progress_new,
                    downloaded, total
                )
                actionProgressText.text = percentage
                actionProgressBar.isIndeterminate = false
        ...
        setOnLongClickListener(getLongClickListener(update, canDelete, this))
        actionStart.visibility = GONE
    }
    ...
    private fun handleNotActiveStatus(update: UpdateInfo) {
        ...
        if (mUpdaterController!!.isWaitingForResume) {
            setOnLongClickListener(getLongClickListener(update, false, this))
            ...
            setOnLongClickListener(getLongClickListener(update, true, this))
